...
var musicData = [
    ['洛春赋','24215220207-肖寒怡'],
    ['Yesterday','24215220207-肖寒怡'],
    ['江南烟雨色','24215220207-肖寒怡'],
    ['Vision pt.II','24215220207-肖寒怡'],
];
...
//初始化音乐
function initAudio() {
    audio.src =`./music/mp3/music${musicId}.mp3`;
    audio.load();
    recordImg.classList.remove('rotate-play');
    audio.onloadedmetadata = function() {
        musicTitle.innerText = musicData[musicId][0];
        author.innerText = musicData[musicId][1];
        recordImg.style.backgroundImage = `url('./images/img/record${musicId}.jpg')`;
        body.style.backgroundImage = `url('./images/img/bg${musicId}.png')`;
...
volumeTogger.addEventListener("input", function() {
    ...
    if (audio.muted) {
        audio.muted = false;
        volume.style.backgroundImage = `url('./images/img/音量.png')`;
    }
});
//滑块音量控制
...
function setNoVolume() {
    ...
    if(audio.muted){
        ...
        volume.style.backgroundImage = `url('./images/img/静音.png')`;
    }else{
...
mode.addEventListener("click", function() {
    ...
    mode.style.backgroundImage = `url('./images/img/mode${modeId}.png')`;
});
